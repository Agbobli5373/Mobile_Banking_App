...
import com.mobilebanking.shared.domain.exception.UserNotFoundException;import com.mobilebanking.transaction.domain.Transaction;
 ...
 * Handles transaction history retrieval with proper filtering and authorization.
 */
@Service
public class TransactionQueryService {
     ...
     * This method ensures the authenticated user can only access their own transaction history.
     *
     ...
     * @throws AccessDeniedException if the requester is not authorized to access the transaction history
     */
    @Transactional(readOnly = true)
    public List<Transaction> getTransactionHistory(UserId userId) {
        ...
        if (!currentUserId.equals(userId)) {
            logger.warn("Unauthorized transaction history access attempt: {} tried to access {}", currentUserId, userId);
